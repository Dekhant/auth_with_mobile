...
class PhoneAuthBloc extends Bloc<PhoneAuthEvent, PhoneAuthState> {
  ...
  Stream<PhoneAuthState> mapEventToState(
    ...
    if (event is PhoneAuthNumberVerified) {
    ...
    } else if (event is PhoneAuthCodeAutoRetrievalTimeout) {
      yield PhoneAuthCodeAutoRetrievalTimeoutComplete(event.verificationId);
      ...
      yield* _passwordConfirmedToState(event);
    }
  }
  ...
  Stream<PhoneAuthState> _phoneAuthCodeVerifiedToState(
    ...
    try {
      ...
      print('Exception occured while verifying OTP code ${e.toString()}');
      yield PhoneAuthCodeVerificationFailure(
          e.toString(), event.verificationId);
    }
  }

  Stream<PhoneAuthState> _passwordConfirmedToState(
      PasswordConfirmed event) async* {
    try {
      yield PhoneAuthLoading();
      UserModel userModel = await _phoneAuthRepository.addUserToCollection(
          email: event.email,
          name: event.name,
          password: event.password,
          phoneNumber: event.phoneNumber);

      yield ConfirmPasswordSuccess(user: userModel);
    } on Exception catch (e) {
      print('Exception occured while verifying OTP code ${e.toString()}');
      yield ConfirmPasswordFailure(
          e.toString(),);
    }
  }
  ...
  void _onCodeAutoRetrievalTimeout(String verificationId) {
    ...
    add(PhoneAuthCodeAutoRetrievalTimeout(verificationId));
  }
}
